...
import { saveWaypointCoordsAction, addMapWaypointAction, deleteMapWaypointAction } from '../actions/schedule.actions'
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-5">
        ...
        {currentRoute && (
          <div className={isFullscreen
            ...
            {/* Zona del mapa (izquierda en fullscreen, full en normal) */}
            <div className={isFullscreen ? "flex-1 h-full relative flex flex-col" : "flex flex-col"}>

              {/* Toolbar: siempre visible encima del mapa */}
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    title={isAddMode ? 'Clic en el mapa para añadir — vuelve a presionar para salir' : 'Activar modo añadir punto'}
                  >
                    <Plus size={14} />
                    {isAddMode ? 'Cancelar' : 'Añadir punto'}
                  </button>

                  {/* Guardar cambios (solo normal view) */}
                  {!isFullscreen && dirtyIds.size > 0 && (
                    <button
                      onClick={handleSave}
                      disabled={isPending}
              ...
              <div className="w-full md:w-96 bg-white border-l border-slate-200 flex flex-col h-full overflow-hidden shadow-2xl relative z-[200]">
                ...
                <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-slate-800 text-sm truncate">{currentRoute.label}</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Editor de ruta</p>
                  </div>
                </div>
                ...
                <div className="px-5 py-4 border-b border-slate-100">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Ej: Av. Tupac Amaru, Cusco"
                      className="flex-1 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 text-slate-700 placeholder:text-slate-300"
                    />
                    <button
                      onClick={handleSearch}
                      disabled={isSearching}
                      className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors disabled:opacity-60 shrink-0"
                    >
                      {isSearching ? (
                  ...
                  {searchResult && (
                    <div className="mt-2 bg-emerald-50 border border-emerald-200 rounded-lg p-2 flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-emerald-600 truncate">{searchResult.label.split(',').slice(0,3).join(',')}</p>
                      </div>
                      <button onClick={() => { setSearchResult(null); setSearchQuery('') }} className="text-emerald-500 hover:text-emerald-700 shrink-0">
                        <X size={14} />
                      </button>
                    </div>
                  )}
                ...
                <div className="flex-1 overflow-y-auto p-5">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Secuencia de paradas</p>
                  <div className="relative">
                    <div className="absolute left-[19px] top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-300 via-blue-100 to-transparent" />
                    <ul className="space-y-4 relative">
                      {(() => {
                        ...
                        const nodes = waypoints.map((w, i) => ({
                          title: i === 0 ? w.originPoint : (i === waypoints.length - 1 ? w.destinationPoint : w.originPoint),
                          isFirst: i === 0,
                          isLast: i === waypoints.length - 1
                        }))

                        if (waypoints.length > 0) {
                          nodes.push({ title: waypoints[waypoints.length - 1].destinationPoint, isFirst: false, isLast: true })
                        }
                        ...
                        return nodes.map((node, i) => (
                          <li key={i} className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0 pt-1">
                              ...
                              <p className="text-sm font-semibold text-slate-700 break-words leading-snug">{node.title}</p>
                            </div>
                          </li>
                        ))
                      })()}
                ...
                <div className="p-5 border-t border-slate-100 bg-slate-50">
                  {message && (
                    <div className={`flex items-center gap-2 p-3 mb-3 rounded-lg text-xs font-medium border ${
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-3 rounded-xl transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    <Save size={18} />
